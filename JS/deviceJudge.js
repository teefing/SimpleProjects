...
const judgeDevice = (target) => {
  ...
  // 判断 移动 PC
  if (
    /Mobile|Android|webOS|iPhone|iPad|iPod|Phone|BlackBerry|IEMobile|OperaMini/i.test(
      us
    )
  ) {
  ...
  if (
    !currentDevice.includes(["Mac"]) &&
    !currentDevice.includes("Win") &&
    pf === "X11"
  ) {
    // unix
